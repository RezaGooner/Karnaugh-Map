...
// مقایسه دو رشته باینری بر اساس تعداد بیت‌های متفاوت یا برابر بودن آنها
...
// تغییر بیت متفاوت به '_' در رشته باینری
string change(string a, int l) {
    a[l] = '_';
    return a;
}

// بررسی اینکه رشته باینری قبلاً در بردار اولیه وجود دارد یا خیر
...
void MainWindow::on_groupButton_clicked()
{
    ...
    if (isMinButtonPushed){
        ...
        if (resultPy.size() ==  0){
            ...
            msgBox.setFont(font);
            msgBox.setWindowTitle("گروه بندی نداریم");

            if (allSelect){
                msgBox.setText("همه مینترم ها انتخاب شده و در یک گروه قرار دارند!");
            } else {
                msgBox.setText("هیچ مینترمی انتخاب نشده!");
            }
            ...
            QStringList parts = resultPy.split("] [", QString::SkipEmptyParts); // جدا کردن بخش‌ها

            int rowCount = parts.size(); // تعداد سطرهای جدول
            int columnCount = 1; // تعداد ستون‌های جدول

            QTableWidget* tableWidget = new QTableWidget(rowCount, columnCount); // ایجاد جدول
            QString columnName = "گروه بندی مینترم ها"  ;
            int column = 0;
            QTableWidgetItem* headerItem = new QTableWidgetItem(columnName);
            tableWidget->setHorizontalHeaderItem(column, headerItem);

            for (int row = 0; row < rowCount; ++row) {
                QString part = parts[row].remove(QRegExp("[\\[\\]']")); // حذف کاراکترهای [ و ] و '

                QTableWidgetItem* item = new QTableWidgetItem(part); // ساخت سلول جدول
                tableWidget->setItem(row, 0, item); // اضافه کردن سلول به جدول
            }

            QDialog dialog; // ایجاد پنجره (QDialog)
            dialog.setWindowTitle("گروه بندی مینترم ها");

            QVBoxLayout* layout = new QVBoxLayout(&dialog); // ایجاد طرح بندی برای پنجره

            layout->addWidget(tableWidget); // اضافه کردن جدول به طرح بندی

            dialog.setLayout(layout);

            // تغییر اندازه ستون
            int columnWidth = 400; // عرض مورد نظر برای ستون (به صورت پیکسل)
            for (int col = 0; col < columnCount; ++col) {
                tableWidget->setColumnWidth(col, columnWidth);
            }

            // تغییر اندازه ردیف‌ها
//          int rowHeight = 200; // ارتفاع مورد نظر برای ردیف (به صورت پیکسل)
  //        for (int row = 0; row < rowCount; ++row) {
    ...
    //      }
        ...
        msgBox.setText("ابتدا تابع را کمینه کنید");
        msgBox.setIcon(QMessageBox::Critical);
        ...
        msgBox.setTextFormat(Qt::RichText); // تنظیم قالب متن به RichText
        msgBox.exec();
    }
}
...
void MainWindow::on_allSelectButton_clicked()
{
    ...
    if (allSelect) {
        ...
        allSelect = false ;
        ui->allSelectButton->setText("همه");
        ...
        allSelect = true ;
        ui->allSelectButton->setText("هیچکدام");
...
void MainWindow::on_saveButton_clicked()
{
    ...
    QString filePath = QFileDialog::getSaveFileName(this, "انتخاب محل ذخیره فایل", "", "فایل‌های متنی (*.txt)");
        ...
        if (file.open(QIODevice::WriteOnly | QIODevice::Text))
        {
            if (resultPy.size() ==  0){
                if (allSelect){
                    ...
                    QMessageBox msgBox;
                    msgBox.setWindowTitle("پیغام");
                    msgBox.setText("فایل با موفقیت ذخیره شد.");
...
void MainWindow::on_dcButton_clicked()
{
    ...
    QMessageBox messageBox(QMessageBox::Question, "dont care", "عملیات مورد نظر را انتخاب کنید:",
                           QMessageBox::Yes | QMessageBox::No | QMessageBox::Cancel);

    messageBox.setButtonText(QMessageBox::Yes, "نمایش");
    messageBox.setButtonText(QMessageBox::No, "تغییر");
    messageBox.setButtonText(QMessageBox::Cancel, "پاک کردن");
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString tempUserInput = QInputDialog::getText(this, "dont cares", "شماره dont care ها را با فاصله وارد کنید:", QLineEdit::Normal, "", &ok);
        QInputDialog inputDialog;
        inputDialog.setFont(font);
        inputDialog.setInputMode(QInputDialog::TextInput);
        inputDialog.setWindowTitle("dont cares");
        inputDialog.setLabelText("شماره dont care ها را با فاصله وارد کنید:");
        inputDialog.setFont(font);
        ...
        if (ok && !userInput.isEmpty()) {
            ...
            // disable checkbox with userInput
            ...
            QMessageBox::information(this, "dont care",  userInput + "\nچک باکس مربوط به آنها غیرفعال شد!");
        } else {
            QMessageBox::critical(this, "خطا", "مقداری (صحیح) وارد نکرده اید!");
        }
        ...
        msgBox.setText("dont care پاک شدند");
...
void MainWindow::on_truetableButton_clicked()
{
    ...
    if (ui->dial->value() == 5) {
        ...
        window->setWindowTitle("جدول درستی");
...
void MainWindow::on_changeVariableButton_clicked()
{
    if (ui->dial->value() == 5) {
        ...
        messageBox.setWindowTitle("تغییر متغیرها");
        messageBox.setText("متغیر های خود را انتخاب کنید");
        ...
        QString selectedValues = "متغیرهای انتخاب شدده برای :\n";
        selectedValues += "بیت پنچم  : " + comboBoxA.currentText() + "\n";
        selectedValues += "بیت چهارم : " + comboBoxB.currentText() + "\n";
        selectedValues += "بیت سوم   : " + comboBoxC.currentText() + "\n";
        selectedValues += "بیت دوم   : " + comboBoxD.currentText() + "\n";
        selectedValues += "بیت اول   : " + comboBoxE.currentText() + "\n";
        ...
        // نمایش مقادیر انتخاب شده در کامبو باکس‌ها
        QString selectedValues = "متغیرهای انتخاب شدده برای :\n";
        selectedValues += "بیت چهارم : " + comboBoxA.currentText() + "\n";
        selectedValues += "بیت سوم   : " + comboBoxB.currentText() + "\n";
        selectedValues += "بیت دوم   : " + comboBoxC.currentText() + "\n";
        selectedValues += "بیت اول   : " + comboBoxD.currentText() + "\n";
        A1 = comboBoxA.currentText().toStdString() + "";
        A0 = comboBoxA.currentText().toStdString()  + "\'" ;
        B1 = comboBoxB.currentText().toStdString()  ;
        B0 = comboBoxB.currentText().toStdString()  + "\'" ;
        ...
        // نمایش مقادیر انتخاب شده در کامبو باکس‌ها
        QString selectedValues = "متغیرهای انتخاب شدده برای :\n";
        selectedValues += "بیت سوم   : " + comboBoxA.currentText() + "\n";
        selectedValues += "بیت دوم   : " + comboBoxB.currentText() + "\n";
        selectedValues += "بیت اول   : " + comboBoxC.currentText() + "\n";
        A1 = comboBoxA.currentText().toStdString() + "";
        A0 = comboBoxA.currentText().toStdString()  + "\'" ;
        B1 = comboBoxB.currentText().toStdString()  ;
        B0 = comboBoxB.currentText().toStdString()  + "\'" ;
        ...
        // نمایش مقادیر انتخاب شده در کامبو باکس‌ها
        QString selectedValues = "متغیرهای انتخاب شدده برای :\n";
        selectedValues += "بیت دوم   : " + comboBoxA.currentText() + "\n";
        selectedValues += "بیت اول   : " + comboBoxB.currentText() + "\n";
        A1 = comboBoxA.currentText().toStdString() + "";
        A0 = comboBoxA.currentText().toStdString()  + "\'" ;
        B1 = comboBoxB.currentText().toStdString()  ;
        B0 = comboBoxB.currentText().toStdString()  + "\'" ;
...
{
    if (ui->indexButton->text() == "حروف"){
        ui->indexButton->setText("بیت");

    } else {
        ui->indexButton->setText("حروف");
...
void MainWindow::on_dial_sliderMoved(int position)
{
    if (position == 2){
        ...
        ui->abCde->setToolTip(QString::fromStdString("در دو متغیره موجود نیست"));
        ui->abCdE->setToolTip(QString::fromStdString("در دو متغیره موجود نیست"));
        ui->abCDe->setToolTip(QString::fromStdString("در دو متغیره موجود نیست"));
        ui->abCDE->setToolTip(QString::fromStdString("در دو متغیره موجود نیست"));
        ...
        ui->AGroupBox->setToolTip(QString::fromStdString("در دو متغیره موجود نیست"));
        ...
        ui->aBcde->setToolTip(QString::fromStdString("در سه متغیره موجود نیست"));
        ui->aBcdE->setToolTip(QString::fromStdString("در سه متغیره موجود نیست"));
        ui->aBcDe->setToolTip(QString::fromStdString("در سه متغیره موجود نیست"));
        ui->aBcDE->setToolTip(QString::fromStdString("در سه متغیره موجود نیست"));
        ui->aBCde->setToolTip(QString::fromStdString("در سه متغیره موجود نیست"));
        ...
        ui->AGroupBox->setToolTip(QString::fromStdString("در سه متغیره موجود نیست"));
        ...
        ui->Abcde->setToolTip(QString::fromStdString("در چهار متغیره موجود نیست"));
        ui->AbcdE->setToolTip(QString::fromStdString("در چهار متغیره موجود نیست"));
        ui->AbcDe->setToolTip(QString::fromStdString("در چهار متغیره موجود نیست"));
        ui->AbcDE->setToolTip(QString::fromStdString("در چهار متغیره موجود نیست"));
        ui->AbCde->setToolTip(QString::fromStdString("در چهار متغیره موجود نیست"));
        ...
        ui->AGroupBox->setToolTip(QString::fromStdString("در چهار متغیره موجود نیست"));
